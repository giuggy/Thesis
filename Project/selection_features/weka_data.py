...
if __name__ == '__main__':
    ...
    path = variable.PATH_EVAL
    for directory in os.listdir(path):
        new_p = path + '/' + directory

        for file in sorted(os.listdir(new_p)):

            if 'data' in file:

                with open(new_p + '/' + file, 'r') as f:
                    data = literal_eval(f.read())
                attacks = data['attack']
                nominal_attacks = []
                for a in attacks:
                    if a == 0:
                        nominal_attacks.append('no')
                    else:
                        nominal_attacks.append('yes')
                data['class'] = attacks
                data.pop('attack')
                df = pandas.DataFrame(data)
                name = file.split('.')
                print(new_p)
                print(pandas2arff(df, filename=new_p + '/' + 'weka_' + name[0] + '.arff', wekaname='class'))
                '''
                arff.dump(new_p + '/' + 'weka_' + name[0] + '.arff'
                          , df.values
                          , relation='attack'
                          , names=df.columns)
