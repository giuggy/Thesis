...
def localization(dictionary, thresh):
    ...
    for sw in dictionary:
        ...
        for lst in dictionary[sw]['host']:
            ...
            result[sw][h] = [('tot', st, score), (s, s1, score1), (d, s2, score2), alarm, s3]
        for lst in dictionary[sw]['port']:
            ...
            if (s1 == 'RED' and s2 == 'RED') or (s1 == 'RED' and s2 == 'ORANGE') or (s1 == 'RED' and s2 == 'YELLOW') \
                ...
                alarm = 'CORRECT'
            result[sw][p] = [('tot', st, score), (s, s1, score1), (d, s2, score2), alarm, s3]
...
def particular_score(switch, dictionary, t):
    ...
    return res, array(res2), t


def verification_network(dictionary, level, f):

    sw_status = {}
    normal = dictionary['normal']
    for status in dictionary:
        if status != 'normal':
            low = level[status]['medium']
            up = level[status]['high']
            print("STATUS", status)
            sw_status[status] = {}
            ...
            for sw in dictionary[status]:
                ...
                if score_conn - n <= low:
                    st = 'GREEN'
                elif low < score_conn - n <= up:
                    st = 'BROWN'
                elif score_conn - n > up:
                    st = 'RED'
                #f.write('SWITCH ' + sw + " STATUS " + str(sw_status[status][sw]) + " " + str(score) + " ONLY CONN " +
                #        str(st) + " " + str(score_conn) + '\n') "STATUS ", sw_status[status][sw],
                print('SWITCH ', sw, score, "ONLY CONN ", st, score_conn)
    return sw_status


def set_network_threshold(normal, thresh):
    low = 0.0
    low_conn = 0.0
    #lst= []
    for sw in normal:
...
def find_threshold(dictionary):
    for key in dictionary.keys():
        if key != 'normal':
            for switch in dictionary['normal']:
                normal = dictionary['normal'][switch]
                attack = dictionary[key][switch]
                score_vec = attack - normal
                print('KMEANS')
                pprint(kmeans(score_vec, 2))


def find_threshold2(dictionary):
    thresh = {}
    for key in dictionary.keys():
        if key != 'normal':
            normal = array(dictionary['normal'])
            attack = array(dictionary[key])
            score_vec = attack - normal
            mat = kmeans(score_vec, 2)
            err = mat[1]
            high = round(mat[0].max() - err, 2)
            medium = round(mat[0].min() + err, 2)
            thresh[key] = {'high': high, 'medium': medium}
    return thresh
...
if __name__ == '__main__':
    ...
    thresh_tot = {'normal': {}, 'att1': {}, 'att2': {}, 'att3': {}, 'att4': {}}
    t = {'normal': [], 'att1': [], 'att2': [], 'att3': [], 'att4': []}
    ...
    for directory in (os.listdir(variables.PATH_ENTROPY)):
        ...
        for status in status_dic.keys():
            information_tot[status][directory], thresh_tot[status][directory], s = particular_score(directory, status_dic[status], [])
            t[status] += s
    ...
    find_threshold(thresh_tot)
    print('--------------------------')
    thresh = find_threshold2(t)
    pprint(thresh)
    #level = set_network_threshold(score_tot['normal'])
    print('SCORE')
    print(score_tot)

    f = open(os.getcwd() + '/result_thresh.txt', 'a')

    sw_status = verification_network(score_tot, thresh, f)

    #part = set_particular_threshold(information_tot['normal'])
